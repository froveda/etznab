...
describe Admin do
  ...
  describe "with a wrong formatted email" do
    let(:object) { build(:admin, email: 'wrongemail') }
    it_behaves_like "validating format", :email
  end

  it "is invalid with a repeated email" do
    create(:admin, email: 'email@example.com')
    invalid_admin = build(:admin, email: 'email@example.com')
  ...
  describe "is invalid without a password" do
    let(:object) { build(:admin, password: nil) }
